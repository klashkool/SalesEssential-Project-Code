...
public class Ts_Asset extends Salesessential_base {
	static WebDriverWait wait = new WebDriverWait(driver, 200);
	static List<HashMap<String, String>> Ts_Asset = ExcelSheetReader.Ts_Asset_data();
	
	public static void Create_Ts_Assets() throws InterruptedException {
...
// To click on Load File
				...
				driver.findElement(By.name("arDescription")).sendKeys(Ts_Asset.get(0).get("Description LoadFile"));
				Thread.sleep(500);
				...
				driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[4]/tag-input/div/div/tag-input-form/form/input"))
						.sendKeys(Ts_Asset.get(0).get("Keywords LoadFile"));
				driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[4]/tag-input/div/div/tag-input-form/form/input"))
						.sendKeys(Keys.ENTER);
				
				// OWNER EMAIL
				
				driver.findElement(By.name("arOwnerEMail")).sendKeys(Ts_Asset.get(0).get("Owner Email"));
				...
				catgry.selectByVisibleText(Ts_Asset.get(0).get("Content Category"));
				
				// Client Facing 
				
				String[] ClientFacing = Ts_Asset.get(0).get("Client Facing LoadFile").split("/", 0);
				...
				String[] Content_Flavour = Ts_Asset.get(0).get("Content Flavour").split("/", 0);
...
//				Assert.assertEquals(Content_FormatList, SelectedContent_FormatList);
				...
				String[] Audience = Ts_Asset.get(0).get("Audience LoadFile").split("/", 0);
				...
				// Seller Role
				String[] Sellerrole_Asset = Ts_Asset.get(0).get("SellerRole LoadFile").split("/", 0);
				...
				String[] SalesStage1 = Ts_Asset.get(0).get("SalesStage LoadFile").split("/", 0);
				...
				// Buyers Journey
				String[] BuyJour = Ts_Asset.get(0).get("Buyersjourney LoadFile").split("/", 0);
				...
				// Geography
				String[] Geo = Ts_Asset.get(0).get("Geography LoadFile").split("/", 0);
				...
				// Industry
				String[] Industry = Ts_Asset.get(0).get("Industry LoadFile").split("/", 0);
...
//				
				String[] SL = Ts_Asset.get(0).get("Service Lines").split("/", 0);
