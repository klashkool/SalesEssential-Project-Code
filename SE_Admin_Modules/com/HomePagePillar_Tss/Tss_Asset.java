...
public class Tss_Asset  extends Salesessential_base {
	static WebDriverWait wait = new WebDriverWait(driver, 200);
	static List<HashMap<String, String>> Tss_Asset = ExcelSheetReader.Tss_Asset_data();
	
	public static void Create_Tss_Assets() throws InterruptedException {
		...
		WebElement AssetClick = driver.findElement(By.linkText("Asset"));
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", AssetClick);

		wait.until(ExpectedConditions.presenceOfElementLocated(By.linkText("Asset")));
		driver.findElement(By.linkText("Asset")).click();
		
		// To click on Create New Button under Assets tabs
				Thread.sleep(10000);
...
// To click on Load File
				...
				WebElement Title = driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[1]/input"));
				((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", Title);
				Thread.sleep(500);
				//driver.findElement(By.name("title")).sendKeys("ShowdocTest " + lastDate);
				driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[1]/input")).sendKeys("AssetLoadFileTest " + lastDate);
				driver.findElement(By.name("arAssetFile")).sendKeys("C:\\Users\\SanthoshNithyaR\\Downloads\\SE\\Test.txt");
				driver.findElement(By.name("arDescription")).sendKeys(Tss_Asset.get(0).get("Description LoadFile"));
				Thread.sleep(500);
				...
				driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[4]/tag-input/div/div/tag-input-form/form/input"))
						.sendKeys(Tss_Asset.get(0).get("Keywords LoadFile"));
				driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[4]/tag-input/div/div/tag-input-form/form/input"))
						.sendKeys(Keys.ENTER);
				
				// OWNER EMAIL
				
				driver.findElement(By.name("arOwnerEMail")).sendKeys(Tss_Asset.get(0).get("Owner Email"));
				...
				catgry.selectByVisibleText(Tss_Asset.get(0).get("Content Category"));
				
				// Client Facing 
				
				String[] ClientFacing = Tss_Asset.get(0).get("Client Facing LoadFile").split("/", 0);
				...
				String[] Content_Flavour = Tss_Asset.get(0).get("Content Flavour").split("/", 0);
...
//				String[] Content_Format = Ts_Asset.get(0).get("Content Format").split("/", 0);
...
//				Assert.assertEquals(Content_FormatList, SelectedContent_FormatList);
				...
				String[] Audience = Tss_Asset.get(0).get("Audience LoadFile").split("/", 0);
				...
				// Seller Role
				String[] Sellerrole_Asset = Tss_Asset.get(0).get("SellerRole LoadFile").split("/", 0);
				...
				String[] SalesStage1 = Tss_Asset.get(0).get("SalesStage LoadFile").split("/", 0);
				...
				// Buyers Journey
				String[] BuyJour = Tss_Asset.get(0).get("Buyersjourney LoadFile").split("/", 0);
				...
				// Geography
				String[] Geo = Tss_Asset.get(0).get("Geography LoadFile").split("/", 0);
				...
				// Industry
				String[] Industry = Tss_Asset.get(0).get("Industry LoadFile").split("/", 0);
...
//				
				String[] SL = Tss_Asset.get(0).get("Service Lines").split("/", 0);
				List<String> SLList = Arrays.asList(SL);
				System.out.println(SLList);

				WebElement SL_view = driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[19]/select"));
				((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", SL_view);
				Thread.sleep(500);

				Select SLBox = new Select(
						driver.findElement(By.xpath("//*[@id='home-pillar-asset-wrapper']/div/div[3]/app-modify-asset/div/form/div[1]/fieldset/div[19]/select")));
				List<WebElement> selectedSL = SLBox.getAllSelectedOptions();

				List<String> SelectedSLList = new ArrayList<String>();
				for (int p = 0; p < selectedSL.size(); p++) {
					String SelectedSLTxt = selectedSL.get(p).getText().trim();
